...
import { useState } from "react";
 function ProductCard({ product }) {
   ...
   const renderRatingStars = (rating) => {
    ...
    return (
          <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} className="w-[20rem] h-[22rem] bg-white rounded-2xl border-1 border-gray-700 shadow-lg overflow-hidden">
           
           <div className={`${hover ? `scale-[0.95] `: `scale-[0.8]`} w-full h-[63%] flex justify-center items-center  transition-all duration-500`}>
            ...
            <div className="p-2">
              <div className=" flex items-center justify-between py-1">
                ...
                <span className="text-black text-lg font-semibold pr-2">Size: {product.size}</span>
              </div>  
              <div className=" flex items-center justify-between py-1">
                <span className="text-black text-lg font-semibold">Rs.{product.price}</span>

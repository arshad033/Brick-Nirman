...
export default function Navbar({ setIsSidebarOpen, setIsLoginOpen, setIsRegisterOpen }) {
  const { user, setUser ,checkSuppliers, setCheckSuppliers , setIsSupplierOpen , searchTerm, setSearchTerm} = useContext(AppContext);
  ...
  return (
    <>
      <header className="max-sm:w-[106%] w-full h-[5rem] bg-gray-900 flex items-center justify-between px-4 max-sm:pr-6 top-0 fixed z-30">
        ...
        <div onClick={()=>navigate('/products')} className="max-sm:hidden relative w-[30%]">
          <FiSearch className="absolute top-3 left-3 text-gray-500" />
          <input value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)} type="text" placeholder="Search" className="pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none w-full" />
        </div>
      ...
      { [
        ...
        <div className="flex items-center space-x-4">
          ...
          {supplierBtn && <button onClick={()=>setIsSupplierOpen(true)} className={` px-5 py-1 border border-gray-400 hover:bg-gray-950 cursor-pointer rounded-lg transition`}>
              Become a supplier
            </button>
          }
          <div onClick={()=>handleProfile()} className={`${UserLoggedIn ? `block`: `hidden`} max-sm:pr-2 flex items-center space-x-2`}>
            <img src="https://randomuser.me/api/portraits/men/1.jpg" alt="User" className="w-8 h-8 rounded-full" />
            <span>{user?.fullName}</span>
          </div>
        </div>
      </header>
      {showMsg && <PopUp_Message message={<p className="flex items-center gap-0.5"><RiErrorWarningLine className="text-xl"/> Please register as a supplier first</p>} color={"warning"} />}
    </>
  );
}